    ...
    <div>
        <Row class="" justify="space-around" v-if="showForm">
            ...
            <!-- <div style="display:inline-block;width:80%">
                <Form label-position="right" :label-width="dataOpton.Numberv || 100" v-show="searchData.length > 0 || btnGroup.length > 0" @submit.native.prevent>
                    <div class="psearchCon">
                        ...
                        <FormItem>
                            ...
                            <Button type="primary" icon="navicon-round" v-if="dataOpton.autoCol" style="" @click="showList">自定义隐藏列</Button>
                            <i-button class="" v-for="(item, index) in btnGroup" :type="item.bgcolor" @click.stop="activeAction(item.type,index,item.url,item.actionName)" :icon="item.icon " :key="'btn'+index" ref="info" key=index style="margin-right: 3px;">
                                <a :id="item.type" style="width:0px;height:0px;"></a>{{item.title}}</i-button>
                        </FormItem>
                    </div>
                </Form>
            </div> -->
            <template>
                <div class="btnGroup" style="display:inline-block;">
                    <i-button v-for="(item, index) in btnGroup" :type="item.bgcolor" @click.stop="activeAction(item.type,index,item.url,item.actionName)" :icon="item.icon " :key="index" ref="info" key=index style="margin-right: 3px;margin-top:40px;">
                        <a :id="item.type" style="width:0px;height:0px;"></a>{{item.title}}</i-button>
                </div>
            </template>
            ...
            <Modal v-model="showDel" @on-ok="handDel" @on-cancel="cancel" :styles="{top: top+'px'}">
                <p>请确定是否删除</p>
            </Modal>
    ...
    methods: {
        ...
        activeAction(type, index, url, actionName) {
            switch (type) {
                ...
                case "export":
                    this.export(
                        url,
                        this.btnGroup[index].exportName || "商品列表"
                    );
                    break;
                case "import":
                    this.import(url);
        ...
        export(url, name) {
            ...
            this.$refs.goodsTable.exportCsv({
                ...
                data: this.tableData2.filter(
                    (data, index) => index < 9 && index > 0
                )
            });
        ...
        remove(index, url, otherParam) {
            ...
            otherParam
                ? this.delGroup(id, url, _otherPar)
                : this.delGroup(id, url);
        },

        getHeight() {
            var height = document.querySelector(".ivu-layout-content")
                .offsetHeight;
            if (
                this.dataOpton.tab &&
                ...
                if (item.key == "action") {
                    ...
                    item.render = (h, params) => {
                        ...
                        renderArr.push(
                            h("img", {
                                attrs: {
                                    src: params.row[params.column.key].split(
                                        ","
                                    )[0],
                    ...
                    item.render = (h, params) => {
                        ...
                        for (var i in item.selectOPtion) {
                            if (
                                ...
                                item.selectOPtion[i].value
                            ) {
                                params.row[params.column.key] =
                                    item.selectOPtion[i].label;
                            }
                        }
                        renderArr.push(
                            h(
                        ...
                        return h("div", [
                            h(
                                "i-switch",
                                {
                                    props: {
                                        value:
                                            params.row[params.column.key] == 1
                                                ? true
                                                : false
                                    },
                                    on: {
        ...
        getInitData(url, cols) {
            ...
            for (var i in cols) {
                if (cols[i].key != "action" && cols[i].type != "selection")
                    select.push(
                        cols[i].key + " as " + cols[i].key.replace(/\./, "-")
                    );
            }
            let data = {
            ...
            this.$ajaxPost(this.dataOpton.url, data).then(res => {
                ...
                this.getList.count =
                    parseInt(res.data.records) || parseInt(res.data.total);
                this.getList.page =
                    parseInt(res.data.page) || parseInt(res.data.current_page);
