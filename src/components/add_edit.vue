                ...
                <Form ref="paramsData"  :model="paramsData"  :rules="validata" :label-width="90">
                 <!-- //params.formList -->
                    <FormItem :label="item.title+':'"  v-for="(item, index) in params.formList" :model="params.formList"  :prop="item.field.replace(/\./,'-')"  :key="index" >
                        ...
                        <!-- Input end-->
						 <!-- password start-->
						                <Input  v-model="paramsData[item.field.replace(/\./,'-')]" type="password"  :placeholder="'请输入'+item.title" style="width:200px"  v-if="item.type=='password'"> </Input>
                        <!-- password end-->
                        ...
                        <!-- RadioGroup end-->
						 ...
						 <!-- v-model="paramsData[item.field.replace(/\./,'-')]"  -->
						 <!-- RadioGroup start-->
                        <upload  v-if="item.type=='uploadFile'"></upload>
                        <!-- RadioGroup end-->
...
export default {
  ...
  methods: {
    ...
    editorHandle(res, file) {
      ...
      this.$refs.myEditor[0].quill.editor.insertEmbed(
        addImgRange.index,
        "image",
        res.data
      );
